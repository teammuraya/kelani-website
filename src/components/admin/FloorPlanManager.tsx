...
/**
 * FloorPlanManager
 * Admin component for uploading a building's floor plan image and drawing
 * clickable unit zones on top of it.
 *
 * Mirrors MasterPlanManager but links zones → project_units instead of project_buildings.
 */
...
import {
  Upload, Loader2, Save, Trash2, X, Info,
...
export default function FloorPlanManager({
  buildingId, projectId, floorPlanUrl: initialUrl,
  floorPlanZones: initialZones = [], units,
}: Props) {
  ...
  // Pending zone (just drawn, needs label + unit link)
  ...
  const uploadToConvex = async (file: File): Promise<string> => {
    ...
    const storageId: Id<'_storage'> = await new Promise((res, rej) => {
      ...
      xhr.onload = () => xhr.status === 200
        ? res(JSON.parse(xhr.responseText).storageId)
  ...
  const updateZoneField = (id: string, field: string, value: any) => {
    setZones(prev => prev.map(z => {
      ...
      if (field === 'status') return { ...z, status: value as ZoneStatus };
      if (field === 'unitId') return { ...z, meta: { ...(z.meta ?? {}), unitId: value || undefined } };
      return z;
    }));
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const zonesPayload: FloorZone[] = zones.map(z => ({
        id: z.id,
        label: z.label,
        points: z.points,
        status: z.status as FloorZone['status'],
        unitId: z.meta?.unitId as Id<'project_units'> | undefined,
      }));
      await updateBuilding({
        id: buildingId,
        floor_plan_url: imageUrl || undefined,
        floor_plan_zones: zonesPayload,
      });
  ...
  return (
    <div className="space-y-5 max-w-5xl">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <h3 className="font-semibold text-gray-900 mb-1">Building Floor Plan Image</h3>
        <p className="text-sm text-gray-500 mb-4">
          Upload the floor plan image for this building. You'll draw unit zones on top of it below.
          Units listed as "Available" will appear in green, "Reserved" in amber, "Sold" in gray.
        </p>
        ...
        {urlMode === 'url' ? (
          <div className="flex gap-3">
            <input
              ...
              placeholder="https://... (floor plan image)"
          ...
          >
            ...
            <p className="text-sm text-gray-500">Click to upload floor plan image</p>
            <p className="text-xs text-gray-400 mt-1">JPG, PNG, WebP — high resolution recommended</p>
          </div>
        )}
        ...
        {imageUrl && (
          <div className="mt-4 relative rounded-xl overflow-hidden border border-gray-200 max-h-40">
            <img src={imageUrl} alt="Floor plan preview" className="w-full h-40 object-cover" />
            <button
          ...
          <div className="p-4 border-b border-gray-100 flex items-center justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                Use the pencil tool (top-right on canvas) to draw polygon zones over each unit.
                Click to add points — double-click or click back on the start point to close the polygon.
                Then link the zone to a unit from the list.
              </p>
            </div>
            <button
              onClick={handleSave}
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="A101, Penthouse, Studio 3…"
                    className={inputCls}
                    autoFocus
                    onKeyDown={e => e.key === 'Enter' && confirmPendingZone()}
                  />
                </div>
                <div>
                ...
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-1">
                    Link to Unit <span className="text-gray-400 font-normal">(click = navigate to unit tour)</span>
                  </label>
                  <select
                    value={pendingUnitId}
          ...
          {zones.length > 0 && (
            <div className="p-4 border-t border-gray-100 space-y-2">
              ...
              {zones.map(zone => (
                <div
                  key={zone.id}
                  className="grid sm:grid-cols-[1fr_auto_1fr_auto] gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100 items-center"
                >
                  <input
                    value={zone.label}
                  ...
                  <select
                    value={zone.meta?.unitId ?? ''}
                    onChange={e => updateZoneField(zone.id, 'unitId', e.target.value)}
                    className={inputCls}
                  >
      ...
      {!imageUrl && (
        <div className="bg-gray-50 rounded-2xl border border-dashed border-gray-200 p-10 text-center text-gray-400">
          ...
          <p className="text-sm">Upload or paste the building floor plan image above to start drawing unit zones</p>
        </div>
      )}
    </div>
  );
