...
/**
 * MasterPlanViewer
 * Full-screen viewer for the project master plan / site plan.
 * Shows phase zones on a canvas (or over a video). Clicking a phase zone
 * navigates to the phase page.
 *
 * Hierarchy: Project → [this page] → Phase → Unit
 */
...
import {
  ArrowLeft, ChevronLeft, ChevronRight, Map, Images, Compass, X,
  ChevronRight as Chevron, Layers,
} from 'lucide-react';
...
function PhaseThumb({ phase, isActive, onClick }: {
...
export default function MasterPlanViewer({ project, phases }: {
  ...
  const [highlightedPhaseZoneId, setHighlightedPhaseZoneId] = useState<string | null>(null);
  ...
  // CSS zoom for transparent-mode canvas (video + canvas zoom together)
  const [masterZoomStyle, setMasterZoomStyle] = useState<React.CSSProperties>({});
  const handleMasterTransparentZoom = useCallback(
  ...
  const handleZoneClick = (zone: CanvasZone) => {
    if (zone.meta?.phaseId) {
      ...
      if (ph) { router.push(`/projects/${project.slug}/phases/${ph.slug}`); return; }
    }
    setHighlightedPhaseZoneId(zone.id);
  };
    ...
    if (tab === 'sitemap' && hasMasterPlan) {
      return (
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute inset-0" style={hasMasterVideo ? masterZoomStyle : {}}>
            {/* Video background */}
            {hasMasterVideo && (
              <video
                src={project.master_plan_video_url!}
            ...
            <div className="absolute inset-0 z-10">
              <ImmersiveCanvas
                ...
                highlightedZoneId={highlightedPhaseZoneId}
                onTransparentZoom={handleMasterTransparentZoom}
      ...
      <div className="hidden md:block absolute top-40 left-8 z-30 w-[340px]">
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <div className="text-white/50 text-[9px] uppercase">Phases</div>
              <div className="text-white text-2xl font-bold">{phases.length}</div>
            </div>
            <div>
              <div className="text-white/50 text-[9px] uppercase">From</div>
          ...
          {hasPhases && (
            <div className="space-y-1 pt-2">
              ...
              {phases.map((ph) => (
                <Link key={ph._id}
                  href={`/projects/${project.slug}/phases/${ph.slug}`}
                  onMouseEnter={() => setHighlightedPhaseZoneId(
                    canvasZones.find(z => z.meta?.phaseId === ph._id)?.id ?? null
                  )}
                  onMouseLeave={() => setHighlightedPhaseZoneId(null)}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-white text-xs font-semibold group-hover:text-white/90">{ph.name}</div>
                    {ph.total_units && (
                      <div className="text-white/40 text-[10px]">{ph.total_units} units</div>
                    )}
                  </div>
          ...
          {tab === 'sitemap' && hasMasterPlan && (
            <div className="text-white/30 text-[10px] italic">
              Click a highlighted zone to explore that phase
            </div>
          )}
              ...
              <div className="flex items-center gap-3">
                {phases.slice(0, 6).map((ph, i) => (
                  <PhaseThumb key={ph._id} phase={ph} isActive={i === activePhaseIdx}
                    onClick={() => {
                      setActivePhaseIdx(i);
                      router.push(`/projects/${project.slug}/phases/${ph.slug}`);
                    }}
                  />
                ))}
              </div>
              <button onClick={() => setActivePhaseIdx(i => Math.min(phases.length - 1, i + 1))}
                className="text-white w-12 h-12 rounded-full hover:bg-white/20 flex items-center justify-center">
      ...
      <div className="md:hidden relative -mt-8 z-20">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-3 p-3 rounded-2xl bg-black/40 backdrop-blur-xl border border-white/10 justify-center overflow-x-auto">
            {tab === 'sitemap' && phases.map((ph, i) => (
              <PhaseThumb key={ph._id} phase={ph} isActive={i === activePhaseIdx}
                onClick={() => router.push(`/projects/${project.slug}/phases/${ph.slug}`)} />
            ))}
      ...
      <div className="md:hidden relative bg-black p-4 pb-10">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3 text-center">
            <div>
              <div className="text-white/50 text-[10px] uppercase">Phases</div>
              <div className="text-white text-xl font-bold">{phases.length}</div>
            </div>
            <div>
          ...
          <div className="space-y-2">
            {phases.map(ph => (
              <Link key={ph._id} href={`/projects/${project.slug}/phases/${ph.slug}`}
                ...
                <span className="text-white text-sm font-medium flex-1">{ph.name}</span>
